...
class Solution:
    def sortList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        if not head or not head.next:
            return head
        first, second = head, self.getmid(head) #head is first because we need to sort both halves of the list, not just the second half if you did self.getmid(head) for both!
        tmp = second.next
        second.next = None #we don't need first = second after this!
        second = tmp
...
class Solution:
    def sortList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        tmp = second.next
        second.next = None
        left, right = self.sortList(first), self.sortList(tmp)
        return self.merge(left, right)
...
#7/21/24 review (my own solution):
...
class Solution:
    def sortList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        sh = second.next
        second.next = None
        first, second = self.sortList(first), self.sortList(sh)
...
#my own solution on 10/19/24 using python3:
