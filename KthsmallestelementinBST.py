...
#7/15/24 refresher (missed yesterday):
    ...
    def kthSmallest(self, root: Optional[TreeNode], k: int) -> int:
        ...
        while cur or stack:
            while cur:
                stack.append(cur)
                cur = cur.left
            #reached leaf node, so become value on top of stack (last seen)
            cur = stack.pop()
            n += 1
            if n == k:
                return cur.val
...
#my own solution using python3 as a review on 10/17/24:
