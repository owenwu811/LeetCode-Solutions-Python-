...
class Codec:
    def serialize(self, root): #to string
        res = []
        def f(root):
            if root == None:
                res.append("N")
    ...
    def deserialize(self, data): #to tree
        arr = data.split(",") #split converts a string back into an array
        self.i = 0
        def f():
            if arr[self.i] == "N":
                self.i += 1
...
class Codec:
    ...
    def deserialize(self, data: str):  # Deserialize the string into a binary tree
        ...
        def f():
            if res[self.i] == "N":
                self.i += 1 #self.i += 1 must go before return None, not after, because return means nothing below that line executes, so self.i += 1 wouldn't even be read
                return None
...
class Codec:
    ...
    def serialize(self, root):
        ...
        f(root)
        return ",".join(res) #we need space seperated or else one big string
