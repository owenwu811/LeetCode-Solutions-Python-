...
class Solution:
    def maximalSquare(self, matrix: List[List[str]]) -> int:
        sidelength = 0 #sidelength must be 0, not 1, or we fail test case matrix = [["0"]] because we would get 1 as the output instead of 0 
        m, n = len(matrix), len(matrix[0])
        dp = [[0] * n for i in range(m)]
        for r in range(m):
            for c in range(n):
...
class Solution:
    def maximalSquare(self, matrix: List[List[str]]) -> int:
        sidelength = 0 #again, sidelength must be 0, not 1, or we fail test case matrix = [["0"]] because we would get 1 as the output instead of 0 !!!!
        m, n = len(matrix), len(matrix[0])
        dp = [[0] * n for i in range(m)]
        for r in range(m):
            for c in range(n):
