...
class Solution:
    from collections import deque
    def levelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        #we want all the values of the tree level by level in a list
        d = deque()
        d.append(root)
        res = []
        while d:
...
class Solution:
    ...
    def levelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        res = []
        #holds values of each node at each level
        d = deque()
        d.append(root)
        while d:
            #add each node of each level of the tree
            level = [] 
            #each level of the tree represents the deque 
            for i in range(len(d)):
                currentnode = d.popleft() 
                if currentnode != None:
                    level.append(currentnode.val)
