...
class RandomizedSet:
    ...
    def remove(self, val: int) -> bool:
        if val in self.mydict:
             #[1, 2] > we want to remove 1 because val = 1
            rear = self.mylist[-1] #2
            indextoinsert = self.mydict[val] #0
            self.mylist[indextoinsert] = rear #
            self.mylist.pop() #[1, 2] > [1]
            self.mydict[rear] = indextoinsert #{2: 1} > {2: 0}
            del self.mydict[val]
            return True
        return False
