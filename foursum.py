...
class Solution:
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        for first in range(len(nums) - 3):
            if first > 0 and nums[first] == nums[first - 1]:
                continue
            for second in range(first + 1, len(nums) - 2): #do not forget first + 1 here!
                if second > first + 1 and nums[second] == nums[second - 1]:
                    continue
                third = second + 1
                fourth = len(nums) - 1
...
class Solution:
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        for first in range(len(nums) - 3):
            if first > 0 and nums[first] == nums[first - 1]:
                continue
            for second in range(first + 1, len(nums) - 2): #first + 1, not first!
                if second > first + 1 and nums[second] == nums[second - 1]:
                    continue 
                third = second + 1
                fourth = len(nums) - 1
...
#9/6/24 review from grokking course (missed again):
...
def four_sum(nums, target):
    ...
    for first in range(len(nums) - 3):
      if first > 0 and nums[first] == nums[first - 1]:
        continue
      for second in range(first + 1, len(nums) - 2): #DO NOT FORGET TO PLACE THE CORRECT POSITION IN THE RANGE FUNCTINO - RANGE(FIRST + 1)!!!!!
        if second > first + 1 and nums[second] == nums[second - 1]:
          continue
        third = second + 1
        fourth = len(nums) - 1
