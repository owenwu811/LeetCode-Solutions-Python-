    ...
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for n in nums:
            if n == 0:
                ...
                continue
            temp = curmax * n
            curmax = max(curmin * n, curmax * n, n) #the ordering matters too - curmax line must come first. if we swapped curmax and curmin lines order, we would fail nums = [2,3,-2,4], outputting 192 instead of 6. this is because we would do curmax = 4 when n = 2 instead of the right way curmax = 2. This is because we would have already computed curmin to be 2, and then curmin * n (2 * 2) would be 4, which is wrong since the curmax at [2] in [2,3,-2,4] is 2 up to that point, not 4
