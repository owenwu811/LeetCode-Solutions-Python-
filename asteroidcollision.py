...
class Solution:
    def asteroidCollision(self, asteroids: List[int]) -> List[int]:
        stack = []
        for a in asteroids:
            while stack and a < 0 and stack[-1] > 0: #don't forget while stack in addition to the other and conditions!
                diff = a + stack[-1]
                if diff < 0:
                    stack.pop()
                elif diff > 0:
...
class Solution:
    def asteroidCollision(self, asteroids: List[int]) -> List[int]:
        stack = []
        for a in asteroids:
            while stack and a < 0 and stack[-1] > 0: #collisions can happen
                diff = a + stack[-1]
                if diff < 0:
                    stack.pop()
                elif diff > 0:
                    a = 0 #because any non zeros are added to stack before any collisions happen
                else:
                    stack.pop()
                    a = 0
            if a != 0:
