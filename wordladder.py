        ...
        while d:
            for i in range(len(d)):
                ...
                for j in range(len(currentword)):
                    pattern = currentword[:j] + "*" + currentword[j + 1:]
                    for n in neighbor[pattern]:
                        if n not in visited:
                            d.append(n)
                            visited.add(n)
...
#6/17/24 review (hesitant, but solved after briefly glancing at solution):
...
class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        for word in wordList:
            for i in range(len(word)):
                pattern = word[:i] + "*" + word[i + 1:]
                nei[pattern].append(word) #{"*it": ["hit"], "h*t": ["hot"]}
        visited = set()
        visited.add(beginWord)
        d = deque()
        d.append(beginWord) #["hit"]
        res = 1
        while d:
            for i in range(len(d)):
                cur = d.popleft() #hit
