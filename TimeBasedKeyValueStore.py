...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        if key not in self.d:
            self.d[key] = [] #[[]] for a[mid][1] to work, not []!
        self.d[key].append([value, timestamp]) #making the [[]]
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        while l <= r:
            ...
            if a[mid][1] <= timestamp:
                res = a[mid][0]
                l = mid + 1 #we want to find the largest that dosen't exceed timstamp_prev
            else:
                r = mid - 1
        return res
...
class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        res = ""
        a = self.d.get(key, []) #[value, timestamp]
        l, r = 0, len(a) - 1
        while l <= r:
            mid = (l + r) // 2
            if a[mid][1] <= timestamp:
...
#7/10/24 review (my own solution):
