...
class MedianFinder:
    ...
    def addNum(self, num: int) -> None:
        heapq.heappush(self.small, -1 * num) #must negate
        if self.small and self.large and (-1 * self.small[0]) > self.large[0]: #must negate
            val = heapq.heappop(self.small)
            heapq.heappush(self.large, -1 * val) #must negate
        if len(self.small) > len(self.large):
            val = heapq.heappop(self.small)
            heapq.heappush(self.large, -1 * val) #must negate
        if len(self.large) > len(self.small):
            val = heapq.heappop(self.large)
            heapq.heappush(self.small, -1 * val) #must negate
 
    def findMedian(self) -> float:
        if len(self.small) > len(self.large):
            return -1 * self.small[0] #must negate
        if len(self.large) > len(self.small):
            return self.large[0]
        return (-1 * self.small[0] + self.large[0]) / 2 #must negate
...
class MedianFinder:
    ...
    def addNum(self, num: int) -> None:
        ...
        if len(self.small) > len(self.large):
            h = heapq.heappop(self.small)
            heapq.heappush(self.large, -1 * h)
        if len(self.large) > len(self.small):
    ...
    def findMedian(self) -> float:
        if len(self.small) > len(self.large):
            ...
            cur = (-1 * self.small[0]) + self.large[0]
            return cur / 2
